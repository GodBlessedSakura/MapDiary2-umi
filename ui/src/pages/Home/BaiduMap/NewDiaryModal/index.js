...
export default function NewDiaryModal(props) {
  ...
  const createDiary = async (user, cb) => {
    ...
    if (response.id) {
      message.success(
        Md2FormatMessage("AddDiarySuccess")
      );
      setFileList([]);
      setContents("");
          ...
          <Modal
            ...
            okText={
              isUploading ? (
                ...
                Md2FormatMessage("Publish")
              )
            }
            onOk={() =>
              createDiary(user, getUserInfo)
            }
            cancelText={Md2FormatMessage(
              "Cancel"
            )}
            centered
            closable={false}
