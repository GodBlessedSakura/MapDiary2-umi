...
export default function DiaryModal(props) {
  const { diary, showDiaryModal, onChangeShowModal } = props
  if (!diary) return
  const { id, title, text, images } = diary
  ...
  return (
    <div>
      <Modal
        ...
        footer={
          <Button type="primary" onClick={() => onChangeShowModal(false)}>
            关闭
          </Button>
        }
      >
        ...
        >
          {images.map((imageUrl, idx) => (
            <Image
              style={{
                objectFit: 'cover',
                // marginLeft: idx % 3 === 0 ? 0 : 10, marginTop: idx - 3 < 0 ? 0 : 10
              }}
              width={200}
              height={200}
              src={imageUrl}
